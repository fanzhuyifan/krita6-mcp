...
Initially rely on Krita's ordinary user undo for validated native strokes. Defer an MCP undo tool until it can target and describe the actual current undo action safely; blindly triggering global Undo could undo the user's intervening work. Return known undo behavior in operation metadata. Report partial mutation honestly if an error occurs after work begins; do not attempt a speculative automatic rollback.
